...
//
const arr1 = [14, 12, 23, 43, 35, 30];
const arr2 = [91, 82, 19, 73, 46, 20];
const arr3 = arr1.concat(arr2);
const oddNumber = arr3.sort((a, b) => {
  return b - a;
});
const SoLe = arr3.filter((num) => num % 2 !== 0);
const Tong = SoLe.reduce((acc, curr) => acc + curr, 0);
//Hàm reducer nhận hai tham số: acc (viết tắt của "accumulator") và curr (viết tắt của "current e
//lement"). acc là giá trị tích lũy từ các vòng lặp trước đó, curr là phần tử hiện tại của mảng đang được xem xét.
const TBSoLe = Tong / SoLe.length;
console.log(TBSoLe);

// let arr = ["t", "c", "e", "j", "b", "o", "-", "y", "a", "r", "r", "a"];
arr[arr.indexOf("-")] = " ";
console.log(arr);
// Xóa phần tử '-' và thêm vào khoảng trắng
arr = arr.map((item) => (item === "-" ? " " : item));

// In ra chuỗi in hoa đảo ngược của mảng
const reversedString = arr.reverse().join("").toUpperCase();
